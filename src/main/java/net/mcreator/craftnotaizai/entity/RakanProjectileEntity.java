...
import net.mcreator.craftnotaizai.procedures.RakanProjectileProjectileHitsLivingEntityProcedure;
...
public class RakanProjectileEntity extends AbstractArrow implements ItemSupplier {
	...
	public void onHitEntity(EntityHitResult entityHitResult) {
		...
		RakanProjectileProjectileHitsLivingEntityProcedure.execute(this.level(), entityHitResult.getEntity(), this);
	}

	@Override
	public void tick() {
		super.tick();
		RakanWhileProjectileFlyingTickProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this.getOwner(), this);
		if (this.inGround)
			this.discard();
	}
