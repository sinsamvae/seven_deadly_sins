...
import net.mcreator.craftnotaizai.procedures.FriesiaOnEntityTickUpdateProcedure;
...
public class FriesiaAnimatedEntity extends Monster implements GeoEntity {
	...
	public FriesiaAnimatedEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(CraftNoTaizaiModEntities.FRIESIA_ANIMATED.get(), world);
	}

	public FriesiaAnimatedEntity(EntityType<FriesiaAnimatedEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
		setMaxUpStep(0.6f);
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "friesia_animated");
	}
	...
	public void baseTick() {
		super.baseTick();
		FriesiaOnEntityTickUpdateProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this);
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(FriesiaAnimatedEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
