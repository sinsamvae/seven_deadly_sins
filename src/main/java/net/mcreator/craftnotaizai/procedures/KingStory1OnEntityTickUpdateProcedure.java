...
public class KingStory1OnEntityTickUpdateProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (entity.getPersistentData().getDouble("skill_cooldown") == 0 && !((entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null) == null)) {
			...
			if (distance <= 6) {
				if (ran == 1) {
					if (entity instanceof KingStory1Entity) {
						((KingStory1Entity) entity).setAnimation("attack_2");
					}
					CraftNoTaizaiMod.queueServerWork(8, () -> {
						{
							...
							if (!projectileLevel.isClientSide()) {
								...
								}.getArrow(projectileLevel, entity, 325, 1);
			...
			entity.getPersistentData().putDouble("skill_cooldown", (Mth.nextInt(RandomSource.create(), 45, 200)));
		}
	}
}
