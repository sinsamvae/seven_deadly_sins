...
public class DianeOnEntityTickUpdateProcedure {
			...
			if (distance <= 6) {
				if (ran == 1) {
					if (entity instanceof DianeAnimatedEntity) {
						((DianeAnimatedEntity) entity).setAnimation("attack_2");
					}
					{
						...
						if (!projectileLevel.isClientSide()) {
							...
							}.getArrow(projectileLevel, entity, 60, 1);
				...
				if (ran == 2) {
					if (!(entity instanceof LivingEntity _livEnt23 && _livEnt23.hasEffect(CraftNoTaizaiModMobEffects.HEAVY_METAL.get()))) {
						if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
							_entity.addEffect(new MobEffectInstance(CraftNoTaizaiModMobEffects.HEAVY_METAL.get(), 1200, 1, false, false));
					}
				}
			}
