...
public class HardProtectionRenderProcedure {
	@OnlyIn(Dist.CLIENT)
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, Entity entity) {
		...
		if ((entity.getCapability(CraftNoTaizaiModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new CraftNoTaizaiModVariables.PlayerVariables())).hard_protection == true) {
			if (_evt.getRenderer() instanceof PlayerRenderer && !(_evt.getRenderer() instanceof com.kleiders.kleidersplayerrenderer.KleidersIgnoreCancel)) {
				...
				if (ResourceLocation.tryParse("craft_no_taizai:textures/entities/hard_protection.png") != null) {
					_texture = new ResourceLocation("craft_no_taizai:textures/entities/hard_protection.png");
				}
