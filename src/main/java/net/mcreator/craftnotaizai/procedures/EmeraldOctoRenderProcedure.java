...
public class EmeraldOctoRenderProcedure {
	@OnlyIn(Dist.CLIENT)
	@SubscribeEvent
		...
		if ((entity.getCapability(CraftNoTaizaiModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new CraftNoTaizaiModVariables.PlayerVariables())).Emerald_Octo) {
			if (_evt.getRenderer() instanceof PlayerRenderer && !(_evt.getRenderer() instanceof com.kleiders.kleidersplayerrenderer.KleidersIgnoreCancel)) {
				...
				if (ResourceLocation.tryParse("craft_no_taizai:textures/entities/gloxiana-vines-texture.png") != null) {
					_texture = new ResourceLocation("craft_no_taizai:textures/entities/gloxiana-vines-texture.png");
				}
				Modelemeraldocto newModel = new Modelemeraldocto(context.bakeLayer(Modelemeraldocto.LAYER_LOCATION));
