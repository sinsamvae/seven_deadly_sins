	...
	public static void execute(LevelAccessor world, Entity entity) {
		...
		}
		{
			...
			for (Entity entityiterator : _entfound) {
				...
				if (entityiterator instanceof Player && !(entityiterator instanceof ServerPlayer _plr15 && _plr15.level() instanceof ServerLevel
						&& _plr15.getAdvancements().getOrStartProgress(_plr15.server.getAdvancements().getAdvancement(new ResourceLocation("craft_no_taizai:defeat_ban"))).isDone())) {
					if (entityiterator instanceof ServerPlayer _player) {
						Advancement _adv = _player.server.getAdvancements().getAdvancement(new ResourceLocation("craft_no_taizai:defeat_ban"));
						AdvancementProgress _ap = _player.getAdvancements().getOrStartProgress(_adv);
						if (!_ap.isDone()) {
