...
public class GilThunder2OnEntityTickUpdateProcedure {
				...
				if (ran == 1) {
					{
						...
						if (!projectileLevel.isClientSide()) {
							...
							}.getArrow(projectileLevel, entity, 210, 1);
					...
					entity.setDeltaMovement(new Vec3((3.5 * entity.getLookAngle().x), (0 * entity.getLookAngle().y), (3.5 * entity.getLookAngle().z)));
					{
						...
						for (Entity entityiterator : _entfound) {
							if (!(entity == entityiterator)) {
								entityiterator.hurt(
										new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(ResourceKey.create(Registries.DAMAGE_TYPE, new ResourceLocation("craft_no_taizai:mana_dmg"))), entity), 210);
