...
public class GrayDemonHendricksonOnEntityTickUpdateProcedure {
			...
			if (distance <= 6) {
				if (ran == 1) {
					if (entity instanceof GrayDemonHendricksonAnimatedEntity) {
						((GrayDemonHendricksonAnimatedEntity) entity).setAnimation("attack_3");
					}
					{
						...
						if (!projectileLevel.isClientSide()) {
							...
							}.getArrow(projectileLevel, entity, 275, 1);
				...
				if (ran == 2) {
					if (entity instanceof GrayDemonHendricksonAnimatedEntity) {
						((GrayDemonHendricksonAnimatedEntity) entity).setAnimation("attack_2");
					}
					if (world instanceof ServerLevel _level)
						_level.sendParticles((SimpleParticleType) (CraftNoTaizaiModParticleTypes.DARK_SNOW.get()), x, (y + 5), z, 100, 5, 5, 5, 0.1);
					{
						final Vec3 _center = new Vec3(x, y, z);
						List<Entity> _entfound = world.getEntitiesOfClass(Entity.class, new AABB(_center, _center).inflate(4 / 2d), e -> true).stream().sorted(Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_center))).toList();
						for (Entity entityiterator : _entfound) {
							if (!(entity == entityiterator)) {
								if ((entityiterator.getCapability(CraftNoTaizaiModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new CraftNoTaizaiModVariables.PlayerVariables())).BP <= 1000) {
									...
									entityiterator.hurt(new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(DamageTypes.GENERIC)), 275);
								}
							}
						}
					}
