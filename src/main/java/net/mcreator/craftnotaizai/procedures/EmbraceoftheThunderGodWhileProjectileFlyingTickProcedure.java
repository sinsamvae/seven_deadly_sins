	...
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, Entity immediatesourceentity) {
		...
		for (int index0 = 0; index0 < (int) (entity.getPersistentData().getDouble("range") * 5); index0++) {
			...
			CraftNoTaizaiMod.queueServerWork((int) delay, () -> {
				...
				if (world instanceof ServerLevel _level)
					_level.sendParticles((SimpleParticleType) (CraftNoTaizaiModParticleTypes.LIGHTNING.get()), (entity.getPersistentData().getDouble("sx") + Math.sin(entity.getPersistentData().getDouble("h")) * (entity.getDirection()).getStepZ()),
