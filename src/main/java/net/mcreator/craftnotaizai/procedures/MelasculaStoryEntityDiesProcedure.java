...
public class MelasculaStoryEntityDiesProcedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (entity.getPersistentData().getDouble("skill_cooldown") == 0 && !((entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null) == null)) {
			...
			if (distance <= 6) {
				if (ran == 1) {
					if (entity instanceof MelasculaStoryEntity) {
						((MelasculaStoryEntity) entity).setAnimation("attack_2");
					}
					CraftNoTaizaiMod.queueServerWork(8, () -> {
						{
							...
							if (!projectileLevel.isClientSide()) {
								...
								}.getArrow(projectileLevel, entity, 340, 1);
										...
										0.75, 0);
							{
								...
								for (Entity entityiterator : _entfound) {
									if (!(entityiterator == entity || (entityiterator instanceof TamableAnimal _tamIsTamedBy && entity instanceof LivingEntity _livEnt ? _tamIsTamedBy.isOwnedBy(_livEnt) : false)
										...
										entityiterator.hurt(
												new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(ResourceKey.create(Registries.DAMAGE_TYPE, new ResourceLocation("craft_no_taizai:mana_dmg")))), 340);
