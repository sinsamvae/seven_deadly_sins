	...
	public static String execute(LevelAccessor world, Entity entity) {
		...
		for (int index0 = 0; index0 < 20; index0++) {
			...
			if (target && entity_target instanceof Player) {
				...
				if (entity instanceof LivingEntity && ((LivingEntity) entity_target).getAttribute(net.minecraft.world.entity.ai.attributes.Attributes.ATTACK_DAMAGE) != null) {
					bp = (entity_target instanceof LivingEntity _attributeContext ? _attributeContext.getAttributeValue(net.minecraft.world.entity.ai.attributes.Attributes.ATTACK_DAMAGE) : 0.0D)
							+ (entity_target instanceof LivingEntity _livEnt ? _livEnt.getArmorValue() : 0) + (entity_target instanceof LivingEntity _livEnt ? _livEnt.getMaxHealth() : -1);
				} else {
					bp = 0 + (entity_target instanceof LivingEntity _livEnt ? _livEnt.getArmorValue() : 0) + (entity_target instanceof LivingEntity _livEnt ? _livEnt.getMaxHealth() : -1);
				}
				return "\u00A7eBattle Power: " + new java.text.DecimalFormat("##.##").format(Math.ceil(bp));
			}
			range = range + 1;
		}
		return BP;
