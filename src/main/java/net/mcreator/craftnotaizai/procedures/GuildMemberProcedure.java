...
public class GuildMemberProcedure {
	public static String execute(Entity entity) {
		...
		member = ((entity.getCapability(CraftNoTaizaiModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new CraftNoTaizaiModVariables.PlayerVariables())).guild_stack).getOrCreateTag()
				.getString(("guild" + 1 * (entity.getCapability(CraftNoTaizaiModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new CraftNoTaizaiModVariables.PlayerVariables())).guild_number));
		if ((member).equals("")) {
			member = "NONE";
		}
		return member;
