...
public class HowzerEntityDiesProcedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		double drop2 = 0;
		{
			...
			for (Entity entityiterator : _entfound) {
				if (((entityiterator.getCapability(CraftNoTaizaiModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new CraftNoTaizaiModVariables.PlayerVariables())).Story).equals("Story13")) {
					if (entityiterator instanceof Player && !(entityiterator == entity)) {
						{
							double _setval = (entityiterator.getCapability(CraftNoTaizaiModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new CraftNoTaizaiModVariables.PlayerVariables())).xp
									+ (Math.ceil(entityiterator.getPersistentData().getDouble("level")) * 0.5 + 380) * ((double) CraftNoTaizaiConfiguration.XP_AMOUNT.get() / 10);
