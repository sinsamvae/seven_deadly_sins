...
public class HowzerEntityDiesProcedure {
	public static void execute(LevelAccessor world, Entity entity, Entity sourceentity) {
		...
		double drop3 = 0;
		double drop2 = 0;
		if (((sourceentity.getCapability(CraftNoTaizaiModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new CraftNoTaizaiModVariables.PlayerVariables())).Story).equals("Story13")) {
						...
						if (entityiterator instanceof Player && !(entityiterator == entity)) {
							{
								double _setval = (sourceentity.getCapability(CraftNoTaizaiModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new CraftNoTaizaiModVariables.PlayerVariables())).xp
										+ (Math.ceil(sourceentity.getPersistentData().getDouble("level")) * 0.5 + 380) * ((double) RandomRaceConfigConfiguration.XP_AMOUNT.get() / 10);
