...
public class StoryBan2EntityDiesProcedure {
	public static void execute(LevelAccessor world, Entity entity, Entity sourceentity) {
		if (entity == null || sourceentity == null)
			return;
		if (((sourceentity.getCapability(CraftNoTaizaiModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new CraftNoTaizaiModVariables.PlayerVariables())).Story).equals("Story26")) {
						...
						if (entityiterator instanceof Player && !(entityiterator == entity)) {
							{
								double _setval = ((sourceentity.getCapability(CraftNoTaizaiModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new CraftNoTaizaiModVariables.PlayerVariables())).xp
										+ Math.ceil(sourceentity.getPersistentData().getDouble("level")) * 0.5 + 835) * ((double) RandomRaceConfigConfiguration.XP_AMOUNT.get() / 10);
