...
public class DalefinalEntityDiesProcedure {
	public static void execute(LevelAccessor world, Entity entity, Entity sourceentity) {
		...
		double drop3 = 0;
		double drop2 = 0;
		if (((sourceentity.getCapability(CraftNoTaizaiModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new CraftNoTaizaiModVariables.PlayerVariables())).Story).equals("Story19")) {
						...
						if (entityiterator instanceof Player && !(entityiterator == entity)) {
							...
							}
							{
								double _setval = (sourceentity.getCapability(CraftNoTaizaiModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new CraftNoTaizaiModVariables.PlayerVariables())).xp
										+ (Math.ceil(sourceentity.getPersistentData().getDouble("level")) * 0.5 + 555) * ((double) RandomRaceConfigConfiguration.XP_AMOUNT.get() / 10);
								sourceentity.getCapability(CraftNoTaizaiModVariables.PLAYER_VARIABLES_CAPABILITY, null).ifPresent(capability -> {
