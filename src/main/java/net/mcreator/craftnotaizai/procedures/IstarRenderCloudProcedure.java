...
public class IstarRenderCloudProcedure {
	...
	private static void buildClouds(CloudStatus cloudStatus, double x, double y, double z) {
		if (cloudBuffer == null || IstarRenderCloudProcedure.cloudStatus != cloudStatus || IstarRenderCloudProcedure.x != x || IstarRenderCloudProcedure.y != y || IstarRenderCloudProcedure.z != z) {
			IstarRenderCloudProcedure.cloudStatus = cloudStatus;
			IstarRenderCloudProcedure.x = x;
			IstarRenderCloudProcedure.y = y;
			IstarRenderCloudProcedure.z = z;
			Minecraft minecraft = Minecraft.getInstance();
	...
	private static void execute(@Nullable Event event, Entity entity) {
		...
		if ((entity.level().dimension()) == ResourceKey.create(Registries.DIMENSION, new ResourceLocation("craft_no_taizai:istar"))) {
			...
			renderClouds(Minecraft.getInstance().options.getCloudsType(), 192, -1, 0, 255 << 24 | 255 << 16 | 0 << 8 | 255);
		}
	}
}
