			...
			for (Entity entityiterator : _entfound) {
				if (entityiterator instanceof Player && !(entityiterator == entity)) {
					...
					if (!(entityiterator instanceof ServerPlayer _plr7 && _plr7.level() instanceof ServerLevel
							&& _plr7.getAdvancements().getOrStartProgress(_plr7.server.getAdvancements().getAdvancement(new ResourceLocation("craft_no_taizai:defeat_king"))).isDone())) {
		...
		if (!CraftNoTaizaiModVariables.MapVariables.get(world).chastiefol) {
			...
			if (drop <= 15) {
				if (world instanceof ServerLevel _level) {
					ItemEntity entityToSpawn = new ItemEntity(_level, x, y, z, new ItemStack(CraftNoTaizaiModItems.CHASTIEFOL_2.get()));
					entityToSpawn.setPickUpDelay(10);
					_level.addFreshEntity(entityToSpawn);
				}
				CraftNoTaizaiModVariables.MapVariables.get(world).chastiefol = true;
